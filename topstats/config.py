...
def apply_theme(root, config):
    """Apply the selected theme to the given window or Tk root."""
    selected_theme = config.get("theme", "dark")

    # sv_ttk requires a Tk instance; allow passing in any widget by
    # resolving its actual root window.
    target = root if isinstance(root, tk.Tk) else root._root()

    sv_ttk.set_theme(selected_theme, target)

    # Ensure the theme background is applied to the requested window
